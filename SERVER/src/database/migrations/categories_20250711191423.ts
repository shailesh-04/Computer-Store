...
class Categories implements IClassCategories {
    ...
    async create(body: ICategories): Promise<any[]> {
        const { demo } = body;
        const result = await database.query(`INSERT INTO categories(demo) VALUES (?)`, [demo]);
        return result;
    }
    async update(id: string, body: ICategories): Promise<any[]> {
        const { demo } = body;
        const result = await database.query(`UPDATE categories SET demo = ? WHERE id = ?`, [demo, id]);
        return result;
    }
